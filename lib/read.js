      ...
      //}, function (err, window) {
        ...
        $('.definition-table tbody').each(function (i, body) {
            ...
            if (atypes.length > 0) {
              ...
              if(types.length == 1){  //expect at least one type
                ...
                for (var i=0;i<types.length;i++)
                        ...
                        var combinedTypeString = types.join(",");
                  multitypes= '{' + combinedTypeString + '}';
                  multitypes=JSON.parse(multitypes);
                prop.type = multitypes; 
              }
          ...
          mprop = [];
            ...
            var mpropObject = '{' + combinedMPropString + '}' ;
            mpropObject = JSON.parse(mpropObject);

            schema.bases[currentType] = mpropObject;
